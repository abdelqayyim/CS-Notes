...
const Editor = (props) => {
  ...
  const deleteImg = (imgSource) => {
    ...
    notes.forEach((component, index) => {
      if(Object.keys(component)[0] === "img" && component.img === imgSource){
        ...
        if (Object.keys(notes[index + 1])[0] === "text" &&
  ...
  const moveComponentHandler = (compoenentindex) => { //move the component's position up withing the array 
    console.log(compoenentindex)
    let notes = getNotes();
    if (compoenentindex !== 0) {
      [notes[compoenentindex], notes[compoenentindex - 1]] = [notes[compoenentindex - 1], notes[compoenentindex]];
  ...
  const fixFormat = (note) => { // note should be in the format note = [""] or [{}]
    ...
    note.forEach((x, index) => {
      if (Object.keys(x)[0] === "text") {
        temp.push(
          <NoteText codeText={x.text.replace(/&gt;/gi, ">").replace(/&lt;(?! )/gi, "< ").replace(/&amp;/gi, "&").replace(/</gi, "<")}
            key={Math.random()}
            language={l}
        ...
        temp.push(
          <NoteText codeText={x.replace(/&gt;/gi, ">").replace(/&lt;(?! )/gi, "< ").replace(/&amp;/gi, "&").replace(/</gi, "<")}
            key={Math.random()}
            language={l}
            onMoveUp={ moveComponentHandler}
            index={ index}>
  ...
  const addImage = () => {
    ...
    reader.onload = (evt) => {
      ...
      if (Object.keys(notes[notes.length-1])[0] === "text" && notes[notes.length-1].text.length !== 0) {
        setNoteD((prev) => [
          ...prev,
          <NoteImage
            ...
            key={Math.random()}
          ></NoteImage>,
          <NoteText index={ notes.length} codeText={""} key={Math.random()} language={l}></NoteText>,
        ]);
      }
      else {
